...
const {
    ...
    cashFlowExport,
    balanceSheetExport,
} = require("../../services/businessOverviewExport");
...
router.post("/cash_flow", async (req, res, next) => {
    let reqBody = {};
    const inputFilterData = {
        dateRange: "object",
        reportBasis: "number",
    ...
    try {
        const data = await cashFlowExport(reqBody, res.locals.id, next);
        if (data == false) {
            return;
        }
        res.json(data);
...
router.post("/balance_sheet", async (req, res, next) => {
    ...
    try {
        const data = await balanceSheetExport(reqBody, res.locals.id, next);
        if (data == false) {
            return;
        }
        res.json(data);
